...
# =======================================================================================================
#     Neo CHIFUMI PROGRAM - USING MINASI ALGORITHM
...
class Game():
	...
	def __init__(self):
		...
		self.node_name ="QBO_Neo_chifumi"
	...
	def marker_roi(self, msg):
		try:
			...
			self.speak_this("erreur dans la fonction markeur r o i. Mon programme est instable, désolé de devoir y mettre fin, "+self.genre)
			pass
	...
	def getNextHit(self):
		...
		for i in reversed(range(len(self.pattern))): 
			self.sequenceAChercher = self.pattern[i:] # from i to end
			self.chaineAnalysee = self.pattern[:i] # from start, during i caracters
			self.lstOccurence = []
			for x in range(0, (len(self.pattern)-len(self.sequenceAChercher))):
				if self.pattern[x:(x+len(self.sequenceAChercher))] ==  self.sequenceAChercher: # if sequence found, adding next try
					self.lstOccurence.append(self.pattern[x+len(self.sequenceAChercher):(x+len(self.sequenceAChercher)+2)])
			self.occurence = self.lstOccurence # prediction (Robot/user!)
			...
			# this function reads all possible next predicted try (humain side), and increments choices each time, to sort the best probability
	...
	def play(self):
		...
		# speaks rules
		self.speak_this("Nous voici dans le jeu interactif pierre papier cizo. Vous me direz à hauteu voix la forme de main souhaitée, et vous meu montrerez la carte correspondante.")
		self.speak_this("Par exemple, si vous voulez faire ciseaux, vous direz a haute voix cizo, et vous meu montrerez la carte cizo.")

		self.speak_this("Je suizassé doué à ce jeu, alors, bonne chance")
		...
		while True:
			...
			if self.coup_letter == "": # no card seen
				if self.bad_step >= 1:
					nothing_seen = ["je vois toujours rien "+self.genre, "a nouveau,  trop de temps", "encore trop tard", "toujours en retard", "essayer d'annoncer votre choix un peu plus vite "+self.genre]
					self.speak_this(random.choice(nothing_seen))
					self.bad_step = 0
				else:
					nothing_seen = ["je vois rien. Nous devons donc recommencer", "vous mettez trop de temps, recommençons", "oupss, trop tard, réessayons"]
					self.speak_this(random.choice(nothing_seen))
					self.bad_step += 1
				continue

			# we feed the pattern list with the last ROBOT choice
			self.pattern+=str(self.robot)

			# we feed the pattern list with the last PLAYER choice
			self.pattern+=self.coup_letter # ex : 'p' for paper

			# extract last turn from pattern list
			self.dernier_coup = self.pattern[-2:]
			# labels :
			if self.pattern[-2:][0] == 'R': # robot
				R = self.nom_des_formes[0]
			if self.pattern[-2:][0] == 'P':
				R = self.nom_des_formes[1]
			if self.pattern[-2:][0] == 'S':
				R = self.nom_des_formes[2]
			if self.pattern[-2:][1] == 'r': # player
			...
			# speak scores only if not equal
			if not self.dernier_coup in self.coups_nuls:
				result = ["j'ai "+str(self.Robot_coins)+" points, et vous "+str(self.Player_coins), \
						"vous avez "+str(self.Player_coins)+" points, et moi "+str(self.Robot_coins), \
						str(self.Robot_coins)+" a "+str(self.Player_coins), \
						str(self.Robot_coins)+" contre "+str(self.Player_coins)]
				self.speak_this(result)
			else : pass

			# random pause before next turn
