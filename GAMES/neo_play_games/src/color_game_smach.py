...
class Game(smach.State):
    ...
    def __init__(self):
        smach.State.__init__(self, outcomes=["QBO_Neo_turn"])
...
class Alternate(smach.State):
    ...
    def __init__(self):
        smach.State.__init__(self, outcomes=['QBO_Neo_turn','human_turn'])
        self.client_speak = rospy.ServiceProxy("/say_fr1", Text2Speach)
...
class QBO_Neo_turn(smach.State):
...
def main():
    ...
    with sm:
        smach.StateMachine.add('GAME', Game(), transitions={'QBO_Neo_turn':'QBO_Neo_TURN'})
        smach.StateMachine.add('QBO_Neo_TURN', QBO_Neo_turn(), transitions={'alternate':'ALTERNATE'})
        smach.StateMachine.add('HUMAN_TURN', human_turn(), transitions={'alternate':'ALTERNATE'})
        smach.StateMachine.add('ALTERNATE', Alternate(), transitions={'QBO_Neo_turn':'QBO_Neo_TURN','human_turn':'HUMAN_TURN'})
...
if __name__ == '__main__':
    try:
        rospy.init_node("play_color_balls")
        main()
